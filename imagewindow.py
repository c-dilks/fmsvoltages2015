...
class ImageWindow(Frame):
    ...
    def display_qt_bitshift(self):
        # Colours keyed by QT bitshift
        colours = {0: "white",
                   1: "cyan",
                   2: "blue",
                   3: "green",
                   ...
                   10: "red"}
        for id, cell in self.cells.iteritems():
            bitshift = cell.qt.bitshift + 5 # add 5 to shift to array index
            self.canvas.itemconfig(id, fill=colours[bitshift])
        self.qtbitshiftlegend = Toplevel()
        self.qtbitshiftlegend.resizable(FALSE, FALSE) # Prevent resizing by the user
        canvas = Canvas(self.qtbitshiftlegend, background='black',
            width=200, height=400)
        canvas.pack()
        for i, colour in colours.iteritems():
            # Add 1 from slot number to give range [1, N]
            # This matches Steve's output format
            Label(canvas, fg=colour, justify=LEFT, font=('Courier', '14'),
                text='Bitshift{:>3}'.format(i-5), background='black').grid(row=i)
